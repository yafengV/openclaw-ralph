---
name: ralph
description: "把 PRD 文本转换成 ralph 的 prd.json 并写入指定项目路径。仅负责 PRD→prd.json，不负责执行 ralph.sh（执行由 ralph_runner 工具负责）。每次调用由用户传入 repo 路径。"
user-invocable: true
---
...
> 前置条件：目标仓库必须已经通过 `proj` skill 初始化，`scripts/ralph/CODEX.md`、`CLAUDE.md` 应该已经存在。
...
## 步骤：将 PRD 文本转换成 prd.json

在 `repoPath` 下执行：

1) 确保目录存在：`mkdir -p scripts/ralph`
2) 把用户提供的 PRD 文本解析为 JSON，写到：`<repoPath>/scripts/ralph/prd.json`
...
- 明确引导用户使用 `ralph_runner` 工具（`ralph_run` 命令或通过 `ralph_runner` 插件）
...
你要从消息里解析出参数并开始执行；若用户要求直接迭代实现，引导用户使用 `ralph_runner` 工具。
